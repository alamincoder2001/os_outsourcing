...
class Sales extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->userType = $this->session->userdata('accountType'); 
    ...
    {
        $this->session->unset_userdata('cheque');
        $data['title'] = "Sales Approve";
        $data['content'] = $this->load->view('Administrator/sales/approve_sale', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'SaleMaster_loadingCost'         => $data->sales->loadingCost,
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'SaleDetails_Discount'      => $cartProduct->salesDiscount,
    ...
    public function getSales()
    {
        ...
        $subDay = date('Y-m-d H:i:s', (strtotime('-1 day', strtotime($now))));

        $sales = array_map(function ($value) use ($subDay) {
            if ($this->userType == 'u') {
                $value->isEditEnable = $value->AddTime > $subDay ? 'yes' : 'no';
            } else {
                $value->isEditEnable = 'yes';
            }
            return $value;
        },  $sales);
...
<?php
    ...
    public function approveSales()
    {
        ...
        try {
            ...
            $this->db->set('Status', 'a')->where('SaleMaster_SlNo', $data->saleId)->update('tbl_salesmaster');

            /*approve Sale Details*/
            $this->db->set('Status', 'a')->where('SaleMaster_IDNo', $data->saleId)->update('tbl_saledetails');

            /*Get Sale Details Data*/
            $saleDetails = $this->db->select('*')->where('SaleMaster_IDNo', $data->saleId)->get('tbl_saledetails')->result();
            ...
            $res = ['success' => true, 'message' => 'Sale approve succcessfully'];
        } catch (Exception $ex) {
            $res = ['success' => false, 'message' => $ex->getMessage()];
        }
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            $returnCount = $this->db->query("select * from tbl_salereturn sr where sr.SaleMaster_InvoiceNo = ? and sr.Status <> 'd'", $sale->SaleMaster_InvoiceNo)->num_rows();
            ...
            /*Get Sale Details Data*/
            $saleDetails = $this->db->select('Product_IDNo, SaleDetails_TotalQuantity,Status')->where('SaleMaster_IDNo', $saleId)->get('tbl_saledetails')->result();
