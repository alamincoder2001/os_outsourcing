...
class Purchase extends CI_Controller
{
        ...
        try {
            ...
            foreach ($data->cart as $product) {
                $returnDetails = array(
                    ...
                    'AddBy'                                => $this->session->userdata("FullName"),
                    'AddTime'                              => date('Y-m-d H:i:s'),
                    'PurchaseReturnDetails_brachid'        => $product->PurchaseDetails_branchID
                );
                ...
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_IDNo, $product->PurchaseDetails_branchID]);
            }
        ...
        try {
            ...
            foreach ($oldDetails as $product) {
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->PurchaseReturnDetails_ReturnQuantity, $product->PurchaseReturnDetailsProduct_SlNo, $product->PurchaseReturnDetails_brachid]);
            }
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            $purchase = array(
                ...
                'PurchaseMaster_loadingCost'    => $data->purchase->loading_cost,
            ...
            foreach ($data->cartProducts as $product) {
                ...
                $previousStock = $this->mt->productStock($product->productId, $product->branchId);

                $inventoryCount = $this->db->query("select * from tbl_currentinventory where product_id = ? and branch_id = ?", [$product->productId, $product->branchId])->num_rows();
                if ($inventoryCount == 0) {
                    $inventory = array(
                    ...
                    $this->db->query("
                        ...
                        where product_id = ? 
                        and branch_id = ?
                    ", [$product->quantity, $product->productId, $product->branchId]);
                }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            foreach ($oldPurchaseDetails as $product) {
                $previousStock = $this->mt->productStock($product->Product_IDNo, $product->PurchaseDetails_branchID);
                ...
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->PurchaseDetails_TotalQuantity, $product->Product_IDNo, $product->PurchaseDetails_branchID]);
    ...
    public function  deletePurchase()
    {
        ...
        try {
            ...
            $purchaseDetails = $this->db->select('Product_IDNo,PurchaseDetails_TotalQuantity,PurchaseDetails_TotalAmount,PurchaseDetails_branchID')->where('PurchaseMaster_IDNo', $data->purchaseId)->get('tbl_purchasedetails')->result();

            foreach ($purchaseDetails as $detail) {
                $stock = $this->mt->productStock($detail->Product_IDNo, $detail->PurchaseDetails_branchID);
            ...
            foreach ($purchaseDetails as $product) {
                $previousStock = $this->mt->productStock($product->Product_IDNo, $detail->PurchaseDetails_branchID);
                ...
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->PurchaseDetails_TotalQuantity, $product->Product_IDNo, $detail->PurchaseDetails_branchID]);
...
<?php
    ...
    public function getMaterialPurchase(){
        ...
        $purchases = $this->db->query("
            ...
            $idClause $supplierClause $dateClause
        ")->result();
